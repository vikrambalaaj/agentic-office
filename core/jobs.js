...
// Graceful fallback when jobs workspace isn't configured.
const NO_JOBS_API = {
  available: false,
  getSchedulerStatus: async () => ({
    ...
    running: false,
    healthy: false,
    message: `Jobs API unavailable: missing ${path.join(JOBS_DIR, "lib/api.js")}`,
  }),
  ...
  getJob: async () => null,
  getJobHistory: async () => [],
  runJob: async () => ({ success: false, error: "Jobs API not available" }),
  pauseJob: async () => ({ success: false, error: "Jobs API not available" }),
  resumeJob: async () => ({ success: false, error: "Jobs API not available" }),
  skipJob: async () => ({ success: false, error: "Jobs API not available" }),
  killJob: async () => ({ success: false, error: "Jobs API not available" }),
  clearCache: () => {},
};
...
async function getAPI() {
  ...
  const apiPath = path.join(JOBS_DIR, "lib/api.js");
  if (!fs.existsSync(apiPath)) {
    if (!missingApiWarned) {
      missingApiWarned = true;
      console.warn(`[Jobs] API module not found, using empty jobs state: ${apiPath}`);
    }
    apiInstance = NO_JOBS_API;
    return apiInstance;
  }

  try {
    const { createJobsAPI } = await import(apiPath);
