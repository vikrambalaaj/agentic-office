...
This project is distributed as a ClawHub skill. After changes are merged to `main`, they need to be published to the registry so users can install/update via `clawhub install agentic-office`.
...
clawhub install agentic-office --workdir /path/to/workspace
```
...
```bash
# Enable all agentic-office debug output
